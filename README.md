...
- フォント色・影は `.env` または YAML で指定できます（`TITLE_COLOR`, `BULLET_COLOR`, `TITLE_SHADOW`, `BULLET_SHADOW`, `SHADOW_OFFSET`）。色は #RRGGBB または #RRGGBBAA（アルファ付き）形式。未指定の文字色は白（#FFFFFF）、影はタイトル #000000B4・本文 #000000A0、オフセットは 2,2 になります。
